...
void getCost(int nodo, int cost, string path, vector<vector<Nodo>> &grafo)
{
	if (nodo != 11)
	{
		...
		sem_wait(semaphores[grafo[nodo][i].id_arista]); // espera a que el semaforo este libre
		path += to_string(nodo_vecino) + " ";			// agrergar id al path
		cost += grafo[nodo][i].cost;
		getCost(nodo_vecino, cost, path, grafo);		// sumar costo
		sem_post(semaphores[grafo[nodo][i].id_arista]); // liberar semaforo
														// getCost(nodo_vecino, cost, path, grafo);
	}
	else
	{
...
int main()
{
	...
	for (int i = 0; i < 20; i++)
	{
		...
		semaphores[i] = sem_open("pSem", O_CREAT | O_EXCL, 0644, random);
	}
